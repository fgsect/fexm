...
def main(invocation: str, queue_dir: str):
    global aflfuzzerprocess
    global SyncQueue
    output_argument_idx = invocation.split(" ").index("-o")
    afl_out_dir = invocation.split(" ")[output_argument_idx + 1]
    ...
    try:
        aflfuzzerprocess = subprocess.Popen(("timeout 5m afl-fuzz " + invocation).split(" "), shell=False)
