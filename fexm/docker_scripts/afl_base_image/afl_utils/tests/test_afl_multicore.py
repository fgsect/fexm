...
test_afl_cmdline = [
    os.path.abspath(os.path.expanduser('~/.local/bin/afl-fuzz')), '-f', '@@', '-t', '200+', '-m', '150', '-Q', '-d',
    '-n', '-x', 'dict/target.dict', '-T banner', '-i',
    './in', '-o', './out'
]
...
test_afl_cmdline3 = [
    os.path.abspath(os.path.expanduser('~/.local/bin/afl-fuzz')), '-f', 'sample_file_027', '-t', '200+', '-m', '150',
    '-Q', '-d', '-n', '-x', 'dict/target.dict',
    '-T banner', '-i', './in', '-o', './out'
]
...
class AflMulticoreTestCase(unittest.TestCase):
    ...
    def test_build_master_cmd(self):
        ...
        expected_master_cmd = os.path.abspath(
            os.path.expanduser('~/.local/bin/afl-fuzz')) + ' -f cur_input_000 -M SESSION000 -- ' + target_cmd.replace(
            '%%', conf_settings['file'] + '_000')
        ...
        expected_master_cmd = os.path.abspath(
            os.path.expanduser(
                '~/.local/bin/afl-fuzz')) + ' -f cur_input_001 -M SESSION001:2/3 -- ' + target_cmd.replace(
    ...
    def test_build_slave_cmd(self):
        ...
        target_cmd = 'testdata/dummy_process/invalid_proc --some-opt %%'
        slave_num = 3
        slave_cmd = os.path.abspath(
            os.path.expanduser('~/.local/bin/afl-fuzz')) + ' -f cur_input_003 -S SESSION003 -- ' + target_cmd.replace(
            "%%", conf_settings['file'] + '_003')
