...
def auto_startup_delay(config_settings, instance_num, resume=True):
    ...
    if resume:
        instance_dir = os.path.join(config_settings["output"],
                                    "{}{:03d}".format(config_settings["session"], instance_num),
                                    "queue")
    else:
...
subprocesses to /dev/null (Default: off). Check 'nohup.out' for further outputs.")
    parser.add_argument("-r", "--redirect", dest="redirect", type=str, default=False, help="For debugging purposes, redirect to specific file.")
        ...
        if "interactive" in conf_settings and conf_settings["interactive"]:
            ...
            if not args.verbose and not args.redirect:
                ...
                with open(args.redirect, "ab") as fp:
                    fuzzer_inst = subprocess.Popen(" ".join(['nohup', cmd]).split(), stdout=fp, stderr=subprocess.STDOUT)
