...
IN_COUNT=$((`find "$IN_DIR" -type f 2>/dev/null | wc -l`))
...
FIRST_FILE=`find "$IN_DIR" -type f | head -1`
...
(
  ...
  if [ "$STDIN_FILE" = "" ]; then
    ...
    for fn in `find "$IN_DIR" -type f`; do

      CUR=$((CUR+1))
      fnshort=$(filename "$fn")
      printf "\\r    Processing file $CUR/$IN_COUNT... "

      "$SHOWMAP" -m "$MEM_LIMIT" -t "$TIMEOUT" -o "$TRACE_DIR/$fnshort" -Z $EXTRA_PAR -- "$@" <"$fn"
    ...
    for fn in `find "$IN_DIR" -type f`; do
      ...
      "$SHOWMAP" -m "$MEM_LIMIT" -t "$TIMEOUT" -o "$TRACE_DIR/$fnshort" -Z $EXTRA_PAR -A "$STDIN_FILE" -- "$@" </dev/null
...
find "$IN_DIR" -type f | xargs filename | sed "s#^#$TRACE_DIR/#" | xargs -n 1 cat | sort | \
...
for fn in `find "$IN_DIR" -type f`; do
  ...
  sed "s#\$# $fn#" "$TRACE_DIR/$fnshort" >>"$TRACE_DIR/.candidate_list"
...
while read -r cnt tuple; do
  ...
  $CP_TOOL "$FN" "$OUT_DIR/$FNSHORT"

  if [ "$((CUR % 5))" = "0" ]; then
    sort -u "$TRACE_DIR/$FNSHORT" "$TRACE_DIR/.already_have" >"$TRACE_DIR/.tmp"
    mv -f "$TRACE_DIR/.tmp" "$TRACE_DIR/.already_have"
  else
    cat "$TRACE_DIR/$FNSHORT" >>"$TRACE_DIR/.already_have"
  fi
