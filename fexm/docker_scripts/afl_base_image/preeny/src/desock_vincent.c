...
int preeny_socket_sync(int from, int to, int timeout)
{
	...
	else if (total_n == 0 && from == 0)
	{
		...
		//printf("synchronization of fd %d to %d shutting down due to EOF\n", from, to);
		//exit(0);
		close(to);
		//printf("Socket close now\n", from, to);
		//exit(0);
		return -1;
	}
...
int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	if (preeny_socket_threads_to_front[sockfd]){
		this_socket = dup(sockfd);
	} else {
		this_socket = original_accept(sockfd, addr, addrlen);
	}
	return this_socket;
}
